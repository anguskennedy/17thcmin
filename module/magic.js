...
export function castMagic(event) {
  ...
  if (actor) {
    invokeMagic(element.dataset.id, actor).then((result) => {
      let attribute = interpolate(
        `bh2e.fields.labels.attributes.${result.attribute}.long`
      );
      ...
      let data = { cast: false, prepared: false };
    ...
    console.error(
      `Failed to locate an actor linked to item id ${element.dataset.id}.`
    );
  }
...
function invokeMagic(magicId, caster) {
  ...
  return attributeTest.roll().then((roll) => {
    result.attribute = attribute;
    result.attributeRoll = attributeTest.total;
    result.formula = attributeTest.formula;
    result.successful =
      result.attributeRoll < caster.system.attributes[attribute];
