...
export default class AttackRollDialog extends Dialog {
  static get defaultOptions() {
    return foundry.utils.mergeObject(super.defaultOptions, { width: 325 });
  }

  constructor(settings) {
    let buttons = {
      attack: {
        callback: () => this._rollAttack(),
  ...
  get modifier() {
    return parseInt(
      this.element[0].querySelector('input[name="modifier"]').value
    );
  }

  get weapon() {
    let weapon = this.actor.items.find(
      (i) => i.type === "weapon" && i.id === this._weaponId
    );
  ...
  static build(event, options = {}) {
    ...
    if (actor) {
      ...
      return renderTemplate(
        "systems/17thcmin/templates/dialogs/attack-roll-dialog.html",
        data
      ).then((content) => {
