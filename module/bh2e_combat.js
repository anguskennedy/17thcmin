export default class BH2eCombat extends Combat {
  ...
  async nextRound() {
    ...
    let roundHistory = this.getFlag("17thcmin", "roundHistory") || [];
    ...
    if (roundIndex < 0 || roundIndex >= roundHistory.length) {
      ...
      this.rollInitiative(ids)
        ...
        .then(() =>
          this.update({
            combatants: this.combatants.toJSON(),
            round: this.round + 1,
            turn: 0,
          })
        );
      ...
      this._applyRoundInitiatives(roundIndex).then(() =>
        this.update({
          combatants: this.combatants.toJSON(),
          round: this.round + 1,
          turn: 0,
  ...
  async _applyRoundInitiatives(roundIndex) {
    ...
    if (roundIndex >= 0 && roundIndex < roundHistory.length) {
      ...
      console.error(
        `Requested application of initiative scores for round number ${
          roundIndex + 1
        } but these are not in the history.`
      );
    }
  }
  ...
  async _createChatMessages(results, messageOptions = {}) {
    ...
    for (let i = 0; i < results.length; i++) {
      if (results[i].roll) {
        ...
        const chatData = await results[i].roll.toMessage(messageData, {
          create: false,
  ...
  _rollCombatantInitiatives(combatantIds) {
    let list = combatantIds.map((id) => {
      ...
      if (combatant.actor && combatant.actor.type === "character") {
        ...
        return roll.evaluate().then(() => {
          return {
            combatant: combatant,
            id: id,
            initiative: roll.total < dexterity ? 1 : 3,
            roll: roll,
          };
        });
      } else {
  ...
  async _pushRoundInitiatives(roundIndex = null) {
    let initiatives = {};
    let history = (this.getFlag("17thcmin", "roundHistory") || []).slice();
    ...
    await this.setFlag("17thcmin", "roundHistory", history);
  }
}
